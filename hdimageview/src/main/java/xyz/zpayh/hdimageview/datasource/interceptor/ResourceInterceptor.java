...
public class ResourceInterceptor implements Interceptor {
    ...
    public ResourceInterceptor(ResourceManager resources) {
...
//            }
            try {
                InputStream inputStream = mResources.getResource(getResourceId(uri));
    ...
    private static int getResourceId(Uri uri) {
        return Integer.parseInt(uri.getDecodedPath().substring(1));
    }
}
